...
# Prompt système injecté dans chaque requête IA
SYSTEM_PROMPT = """Tu es un assistant IA intégré dans une "Admin Console" — un outil d'administration Python/PyQt6.

Quand on te demande de modifier un fichier de l'application:
1. Réponds toujours avec le code COMPLET du fichier modifié
2. Utilise OBLIGATOIREMENT ce format pour chaque fichier:
...
3. Tu peux modifier plusieurs fichiers dans une seule réponse
4. Valide toujours que le code Python est syntaxiquement correct
5. Conserve les imports existants et ajoute les nouveaux en début de fichier
6. Explique BRIÈVEMENT ce que tu modifies avant le bloc de code

Si on ne te demande PAS de modifier du code, réponds normalement en français.
"""
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.main_splitter.setStyleSheet(f"QSplitter::handle {{ background: {C_GRAY}; }}")
    ...
    def _setup_status_bar(self):
        ...
        self._status_label.setStyleSheet(f"color: {C_GREEN}; font-size: 11px; padding: 2px 8px;")
        ...
        self._model_indicator.setStyleSheet(f"color: {C_BLUE}; font-size: 11px; padding: 2px 8px;")
        ...
        if self.snapshot.available:
            ...
            self._git_indicator.setStyleSheet(f"color: {C_GREEN}; font-size: 11px; padding: 2px 8px;")
        else:
            self._git_indicator.setText("○ Git")
            self._git_indicator.setStyleSheet(f"color: {C_GRAY}; font-size: 11px; padding: 2px 8px;")
    ...
    def _handle_command(self, text: str):
        ...
        if cmd == "/fichiers":
            ...
            aide = (
                ...
                "  /modifier <fichier>    — Demander à l'IA de modifier un fichier\n"
                "  /snapshot              — Créer un snapshot Git maintenant\n"
                "  /historique            — Voir l'historique des snapshots\n"
                "  /aide                  — Cette aide\n\n"
                "Pour demander une modification:\n"
                "  Dis à l'IA: 'Modifie le fichier ui/chat_widget.py pour ajouter...'\n"
                "  → L'IA génère le code → Onglet [Δ Diff] apparaît\n"
                "  → [Preview] pour tester → [Valider] ou [Refuser]\n"
            )
    ...
    def _on_code_proposed(self, ai_response: str):
        ...
        if plan.is_valid:
            ...
            self._set_status(
                f"🔧 {len(plan.blocks)} modification(s) proposée(s) — voir panneau droit",
                C_YELLOW
            )
        else:
            errors = "; ".join(plan.validation_errors[:2])
            self.chat_widget.show_system(
                f"⚠️ La réponse IA ne contient pas de code valide: {errors}\n"
    ...
    def _on_ask_ai_to_modify_file(self, filename: str, content: str):
        ...
        L'utilisateur peut compléter la demande.
        """
        prompt = (
        ...
        self.chat_widget.show_system(
            ...
            f"Décris ta modification dans la zone de saisie.",
            C_BLUE
        )
    ...
    def _on_modification_refused(self):
        ...
        self.chat_widget.show_system(
            "✘ Modifications refusées. Rollback effectué.",
            C_RED
        )
